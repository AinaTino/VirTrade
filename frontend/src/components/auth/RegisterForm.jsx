  ...
  return (
    <div className="mx-auto max-w-md rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl shadow-black/20">
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <button className="w-full rounded-full bg-cyan-500 px-4 py-3 font-medium text-slate-950 transition hover:bg-cyan-400" type="submit">Créer un compte</button>
      </form>
      <div className="mt-4 text-sm text-slate-400">
        Vous avez déjà un compte ? <Link className="text-cyan-400 hover:text-cyan-300" to="/login">Se connecter</Link>
      </div>
    </div>
  );
}
